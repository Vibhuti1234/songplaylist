...
public class FlipkartApplicationController {
	...
	public String getShirts(@RequestParam("brandId") int brandId ,Model model) throws FlipkartShoppingApplicationException
	{     s=brandId;
		model.addAttribute("productshirt", productService.getShirtsByBrand(brandId));
		return "displayButtons";
	}
	@GetMapping("/getJackets")
	public String getJackets(Model model) throws FlipkartShoppingApplicationException
	{
		model.addAttribute("productjacket", productService.getJacketsByBrand(s));
		return "displayButtons";
	}
	*/
}
